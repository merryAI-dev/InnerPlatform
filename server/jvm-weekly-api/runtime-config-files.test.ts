...
describe('JVM weekly API runtime config files', () => {
  ...
  it('keeps stage deployment database and service-token authority out of frontend config', () => {
    ...
    expect(cloudBuild).toContain('_JVM_WEEKLY_FIREBASE_PROJECT_ID or PROJECT_ID is required for browser-direct auth');
    ...
    expect(deployScript).toContain('JVM_WEEKLY_ALLOWED_ORIGINS="${JVM_WEEKLY_ALLOWED_ORIGINS:-https://inner-platform.vercel.app}"');
