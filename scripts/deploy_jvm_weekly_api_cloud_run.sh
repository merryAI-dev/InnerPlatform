...
DEFAULT_AUTH_MODE="strict"
if [[ "$PROJECT_ID" == "$STAGE_GCP_PROJECT_ID" ]]; then
  DEFAULT_AUTH_MODE="internal_saas_workspace"
fi
JVM_WEEKLY_AUTH_MODE="${JVM_WEEKLY_AUTH_MODE:-$DEFAULT_AUTH_MODE}"
JVM_WEEKLY_WORKSPACE_EMAIL_DOMAIN="${JVM_WEEKLY_WORKSPACE_EMAIL_DOMAIN:-mysc.co.kr}"
DEFAULT_PROJECT_ACCESS_BACKEND="firestore"
if [[ "$PROJECT_ID" == "$STAGE_GCP_PROJECT_ID" ]]; then
  DEFAULT_PROJECT_ACCESS_BACKEND="disabled"
fi
JVM_WEEKLY_PROJECT_ACCESS_BACKEND="${JVM_WEEKLY_PROJECT_ACCESS_BACKEND:-$DEFAULT_PROJECT_ACCESS_BACKEND}"
JVM_WEEKLY_FIREBASE_PROJECT_ID="${JVM_WEEKLY_FIREBASE_PROJECT_ID:-$PROJECT_ID}"
if [[ -z "${JVM_WEEKLY_FIREBASE_AUTH_PROJECT_ID:-}" && "$PROJECT_ID" == "$STAGE_GCP_PROJECT_ID" ]]; then
  JVM_WEEKLY_FIREBASE_AUTH_PROJECT_ID="$STAGE_FIREBASE_AUTH_PROJECT_ID"
else
  JVM_WEEKLY_FIREBASE_AUTH_PROJECT_ID="${JVM_WEEKLY_FIREBASE_AUTH_PROJECT_ID:-}"
fi
...
if [[ "$PROJECT_ID" == "$STAGE_GCP_PROJECT_ID" && "$JVM_WEEKLY_FIREBASE_AUTH_PROJECT_ID" != "$STAGE_FIREBASE_AUTH_PROJECT_ID" ]]; then
  echo "[deploy-jvm-weekly-api] stage must verify Firebase Auth tokens from $STAGE_FIREBASE_AUTH_PROJECT_ID"
  exit 1
fi
