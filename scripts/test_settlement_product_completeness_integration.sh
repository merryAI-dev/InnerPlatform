...
if command -v lsof >/dev/null 2>&1; then
  if lsof -iTCP:"$FIRESTORE_PORT" -sTCP:LISTEN >/dev/null 2>&1; then
    ...
    if [ -z "${ALT_PORT:-}" ]; then
      printf "[settlement-product-completeness] Firestore emulator port %s is in use and no fallback port was found.\n" "$FIRESTORE_PORT"
      exit 1
    fi
    printf "[settlement-product-completeness] Firestore emulator port %s is busy. Using %s instead.\n" "$FIRESTORE_PORT" "$ALT_PORT"
    FIRESTORE_PORT="$ALT_PORT"
  fi
fi

TMP_CONFIG="$(mktemp "$ROOT_DIR/.firebase-settlement-product-XXXX.json")"
...
printf "[settlement-product-completeness] Running Firestore emulator integration tests (project=%s, port=%s)\n" "$PROJECT_ID" "$FIRESTORE_PORT"

npx firebase-tools emulators:exec \
  --only firestore \
  --project "$PROJECT_ID" \
  --config "$TMP_CONFIG" \
  "npx vitest run src/app/data/portal-store.integration.test.ts src/app/data/cashflow-weeks-store.integration.test.ts"
