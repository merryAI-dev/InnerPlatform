...
node -e "const fs=require('fs');const cfg=JSON.parse(fs.readFileSync('firebase.json','utf8'));cfg.emulators=cfg.emulators||{};for(const [name,port] of [['firestore',process.argv[2]],['auth',process.argv[3]],['storage',process.argv[4]]]){cfg.emulators[name]=cfg.emulators[name]||{};cfg.emulators[name].port=Number(port);}fs.writeFileSync(process.argv[1], JSON.stringify(cfg, null, 2));" "$TMP_CONFIG" "$FIRESTORE_PORT" "$AUTH_PORT" "$STORAGE_PORT"
