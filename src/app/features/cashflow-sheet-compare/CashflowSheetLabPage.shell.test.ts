...
describe('CashflowSheetLabPage shell', () => {
  ...
  it('uses the lab BFF client without exposing legacy cashflow write actions', () => {
    ...
    expect(pageSource).toContain('시트 권한 추가하기');
    expect(pageSource).toContain('위 공유계정 확인을 누르고 공유계정 복사를 눌러서 시트에 엑세스 권한을 업데이트 해요.');
    expect(pageSource).not.toContain('아래 공유계정 확인을 누르고 공유계정 복사를 눌러서 시트에 엑세스 권한을 업데이트 해요');
