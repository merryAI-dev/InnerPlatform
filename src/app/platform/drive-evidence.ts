...
export function buildDriveTransactionFolderDescriptor(
  ...
  const counterpartyToken = normalizeSegment(tx.counterparty || '', '');
  const transactionToken = '';
  const folderName = [dateToken, budgetToken, subBudgetToken, counterpartyToken].filter(Boolean).join('_');
...
const CATEGORY_PATTERNS: Array<{ category: string; patterns: RegExp[] }> = [
  ...
  { category: '견적서', patterns: [/견적서/i, /견적\b/i, /quotation/i, /estimate/i] },
  ...
  { category: '세금계산서', patterns: [/전자\s*세금\s*계산서/i, /세금\s*계산서/i, /tax\s*invoice/i, /invoice/i] },
  ...
  { category: '이체확인증', patterns: [/계좌\s*이체\s*확인증/i, /이체\s*확인증/i, /이체\s*확인서/i, /transfer\s*confirmation/i] },
  { category: '입금확인증', patterns: [/입금\s*확인증/i, /입금증/i, /deposit\s*confirmation/i] },
  { category: '입금확인서', patterns: [/입금\s*확인서/i, /송금\s*확인/i, /송금\s*확인증/i, /deposit/i] },
  { category: '재단 메일', patterns: [/재단\s*메일/i, /foundation\s*mail/i, /email/i] },
  { category: '정산규정', patterns: [/정산\s*규정/i, /policy/i] },
  { category: '지출결의', patterns: [/지출\s*결의서?/i, /품의서/i] },
  ...
  { category: '거래명세서', patterns: [/거래\s*명세(?:서|표)?/i, /거래\s*내역서/i, /statement/i] },
  { category: '참석자명단', patterns: [/참석\s*확인서/i, /참석자\s*리스트/i, /참석자\s*명단/i, /출석부/i, /attendance/i] },
