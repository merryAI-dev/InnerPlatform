...
export function buildProjectProposalPost(
  ...
  const body = [
    ...
    `- 총매출부가세: ${formatStoredProjectAmount(draft.salesVatAmount, draft.financialInputFlags?.salesVatAmount)}`,
    `- 총수익: ${formatStoredProjectAmount(draft.totalRevenueAmount, draft.financialInputFlags?.totalRevenueAmount)}`,
    `- 총지원금: ${formatStoredProjectAmount(draft.supportAmount, draft.financialInputFlags?.supportAmount)}`,
