...
- Batch 2.5:
  - 정산 시트의 각 셀에 노션식 `셀 메모` 버튼이 생긴다.
  - 메모는 우측 사이드 패널에서 스레드처럼 이어서 남길 수 있다.
  - 기존 거래 행은 거래 ID 기준으로, 미저장 신규 행은 `sheet-row:*` 기준으로 메모가 유지된다.
  - 메모 패널을 연 상태에서도 시트 편집과 키보드 이동이 계속 가능하다.
- Batch 3:
...
8. 아무 셀 우측 상단의 메모 아이콘을 눌렀을 때 우측 `셀 메모` 패널이 열리고, 기존 메모가 있으면 스레드가 보이는지 확인한다.
9. 메모 패널을 연 상태에서 다른 셀을 클릭하거나 `Enter`로 이동해도 시트 편집이 막히지 않는지 확인한다.
10. 방금 삽입한 신규 행의 셀에서도 메모를 추가하고, 새로고침 후 다시 열었을 때 메모가 남아있는지 확인한다.
11. 상단 `주차 보기` 탭으로 전환했을 때 주차별 섹션이 보이는지 확인한다.
12. 기존 CSV를 업로드한 뒤 `사업비카드/개인법인카드`, `공급가액` 보조 문구, 행 삽입, 셀 메모가 함께 동작하는지 확인한다.
13. 재경 계정으로 `은행 거래내역 대조`를 열고 하나은행 CSV를 업로드한다.
14. 상단 policy 카드에 `하나은행 빠른조회`, `열 메뉴`, `원문 적요 확인`이 보이는지 확인한다.
15. 같은 화면에서 1행 `은행 적요`가 원문 그대로 보이는지 확인한다.
16. PM 계정으로 같은 CSV를 업로드하고 policy 카드에 `마스킹 적요 확인`이 보이는지 확인한다.
17. PM 화면에서 1행 `은행 적요`가 `권한 필요`로 보이는지 확인한다.
18. 재경 계정으로 국민은행 CSV를 업로드하고 policy 카드가 `국민은행 빠른조회`로 바뀌는지 확인한다.
19. 재경 계정으로 신한은행 CSV를 업로드하고 policy 카드가 `신한은행 빠른조회`로 바뀌는지 확인한다.
...
- 셀 메모는 헤더를 추가하지 않고 별도 `comments` 컬렉션에 저장되며, 셀 위치는 `transactionId/sheet-row + fieldKey` 조합으로 구분한다.
