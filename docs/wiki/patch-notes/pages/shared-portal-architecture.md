...
- [2026-08-12] 포털 스토어가 참여율 목데이터로 초기화되던 것을 없앴다. 목 프로젝트 id(`eco26` 등)와 실제 프로젝트 id(`p177…`)는 겹치지 않아 프로덕션에서는 항상 빈 목록이었고, 번들에는 직원 실명만 실려 나갔다. 인사 공지의 기본값이던 가짜 퇴사·전배 공지도 제거했다 — 인증 전 화면에 실존 인물의 없는 일이 사실처럼 떴다.
