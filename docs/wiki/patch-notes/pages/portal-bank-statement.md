...
- [x] 통장 입출금액을 위자드 초안에서 공급가액/매입부가세 10% 포함 공식으로 분해
...
- [2026-06-10] 통장내역에서 선택 행 반영 위자드로 넘어갈 때 입출금액을 부가세 포함 금액 초안으로 분해한다. 출금은 `사업비 사용액 + 매입부가세`, 입금은 `입금액 + 매입부가세 반환`으로 채우며, 공식은 총액 기준 부가세 `총액/11`, 공급가액 `총액-부가세`를 사용한다. 이 값은 Temp 초안이며 최종 반영은 Java command API 검증을 거친다.
