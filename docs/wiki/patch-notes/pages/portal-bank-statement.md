...
- [2026-06-10] 선택 행 반영 Temp 위자드의 `cashflow항목`을 공용 cashflow line policy 옵션으로 고정했다. 이 값은 Actual/회계 연동의 기준키이므로 자유입력을 허용하지 않고, `해당 주차`는 거래일 기준 자동 계산 항목으로 남겨 위자드 수동 입력에서 제외한다.
