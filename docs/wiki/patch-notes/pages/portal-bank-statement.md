...
- 저장된 통장내역은 프로젝트별 append-only 기준본이다.
- 기준본 저장은 주간 사업비 행을 자동 생성하지 않고, 사용자가 선택한 거래만 명시적으로 반영한다.
...
- [x] 선택한 거래만 `사업비 입력(주간)`으로 반영 가능
- [x] 기준본 저장과 주간 사업비 반영 write path 분리
- [x] 선택 행 반영 전 비어 있는 사업비 항목을 Temp 위자드에서 보완 가능
- [x] 선택 행 반영 위자드 진입 전 통장내역 기준본 자동 저장
- [x] Temp 위자드가 예산 편집의 비목/세목/세세목 계층과 필수증빙자료 매핑을 상속
- [x] 통장 입출금액은 위자드 초안에서 원액을 유지하고, 공급가액/매입부가세 추정값은 사용자가 명시적으로 적용
- [x] 거래일시(초 단위) + 거래처 + 금액 기준 중복 제외
...
- [2026-06-10] 통장내역에서 선택 행 반영 위자드로 넘어갈 때 입출금액 원액을 `사업비 사용액` 또는 `입금액`에 그대로 둔다. 부가세 포함 거래의 공급가액/매입부가세 추정값은 `부가세 추정 적용` 버튼으로만 채우며, 자동확정하지 않는다.
- [2026-06-10] 선택 행 반영 Temp 위자드의 `cashflow항목`을 공용 cashflow line policy 옵션으로 고정했다. 이 값은 Actual/회계 연동의 기준키이므로 자유입력을 허용하지 않고, `해당 주차`는 거래일 기준 수정불가 계산값으로 표시한다.
- [2026-06-10] 선택 행 반영 Temp 위자드의 상시 노출 정책 문구를 제거하고, `작성 기준` hover 도움말로만 제공해 입력 화면 밀도를 유지한다.
- [2026-06-10] 선택 행 반영 Temp 위자드의 좌측 요약을 `<<`/`>>` 서랍으로 접을 수 있게 하고, 위험한 전체 일괄적용을 금액 제외 `분류만 복사`로 제한했다. 부가세 추정 버튼은 제거해 행 높이를 줄였다.
- [2026-06-10] `비어있는 사업비 항목 작성` Temp 위자드에 30일 보관 임시저장, 작성 시각 기반 작성본 불러오기, 거래처 이전 조합 제안, 선택 행 일괄적용, Ctrl/Command+Z 되돌리기, 오류 요약을 추가했다. 위자드 진입 시 dirty 통장 원본은 기준본으로 자동 저장하고, 필수증빙자료는 예산 편집의 `비목 > 세목 > 세세목` 매핑을 우선 상속한다.
- [2026-06-10] 선택 행 반영이 원장에 바로 쓰지 않고 `비어있는 사업비 항목 작성` Temp 위자드를 먼저 열도록 바꿨다. 위자드 입력값은 확정 전까지 원본 통장내역과 주간 사업비 원장을 오염시키지 않고, 확정 시 선택 행의 셀 패치만 Java command API로 전달한다.
- [2026-06-10] 통장내역 저장을 append-only 기준본으로 바꾸고, 같은 거래일시(초 단위)·거래처·금액 행은 중복으로 제외한다. 기준본 저장은 주간 사업비를 자동 생성하지 않고, 선택 행 반영 버튼만 주간 사업비 handoff를 수행한다.
...
- 이 화면은 자체 완결 화면이 아니라 `사업비 입력(주간)`의 원본 기준본 역할을 한다.
- 기준본 저장은 원본 stack 보존만 수행한다. 주간 사업비 반영은 사용자가 선택한 행에 대해서만 별도 action으로 수행한다.
- 선택 행 반영 위자드는 Temp/dirty 작업 공간이며, 확정 전까지 원본 통장내역과 사업비 입력 원장을 수정하지 않는다.
- 중복 판단은 카드/계좌/잔액/메모가 아니라 거래일시(초 단위), 거래처, 금액만 본다.
