...
- [x] 입력 자동 저장 루프를 끄고 Projection/Actual 확인은 포털 캐시플로 화면으로 분리
- [x] 주간 사업비 입력에서 포털 캐시플로 화면으로 바로 이동 가능
- [ ] 입력 보조 드롭다운/팝오버 잘림 이슈 완전 해소 확인 필요

## Recent Changes

- [2026-06-10] 주간 사업비 입력 화면에서 cashflow read model 요약과 강제 갱신을 제거했다. 화면 흔들림을 줄이기 위해 주간 입력은 저장/입력 표면으로 유지하고, Projection/Actual 확인은 포털 캐시플로 화면 이동으로 분리했다.
- [2026-06-09] 주간 사업비 입력 화면 상단에 포털 캐시플로 화면으로 바로 이동하는 액션을 추가해, 입력 저장 후 Projection/Actual 비교 화면으로 곧바로 넘어갈 수 있게 했다.
- [2026-06-09] 사업비 입력(주간)의 1.5초 자동 저장 루프를 끄고 수동 저장 정책으로 전환했다.
