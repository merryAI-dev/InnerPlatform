...
- [x] 입력 자동 저장 루프를 끄고 명시 저장 후 Java cashflow read model을 다시 읽어 Projection/Actual 요약 표시
...
- [2026-06-09] 사업비 입력(주간)의 1.5초 자동 저장 루프를 끄고 수동 저장 정책으로 전환했다. 저장 성공 후에는 Java cashflow read model snapshot을 강제 갱신해 같은 화면에서 Projection, Actual, 차이 요약을 확인할 수 있게 했다.
