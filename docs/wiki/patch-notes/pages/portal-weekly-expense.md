...
- [x] 원장 조회 화면에서 저장 가능/미입력/수정 필요 검증 배지 미노출
...
- [2026-06-10] 사업비 입력(주간)을 완전 조회 전용으로 고정했다. `상세 적요` 예외 편집을 제거하고, read-only 원장에서는 `저장 가능`, `수정 필요`, `미입력` 배지가 뜨지 않도록 했다.
...
- `해당 주차`와 `cashflow항목`은 원장에 들어온 뒤 fixed 값이다. Cashflow Actual 반영은 사용자가 이 화면에서 저장/동기화하지 않는다.
