...
- 홈 본문에는 상태 유무와 관계없이 `이번 달 인건비 확인` CTA가 항상 보여, 인건비/공지 화면을 숨기지 않고 바로 찾을 수 있다.
...
- [x] 홈에서 `이번 달 인건비 확인` CTA를 통해 `/portal/payroll`로 항상 이동 가능
...
- [2026-06-19] `/portal/cashflow`에서는 HR/Payroll provider를 더 이상 mount하지 않고, shell badge/notification 계산은 optional hook의 빈 상태를 사용하게 해 Cashflow 진입 시 무관한 provider fetch와 권한 오류가 화면을 흔들지 않도록 정리했다.
- [2026-05-20] 포털 shell과 프로젝트 선택/배정 표기를 `내 프로젝트`, `프로젝트 전환`, `담당 프로젝트`, `계약 대상` 기준으로 정리하고, 새 프로젝트 등록과 기존 프로젝트 수정이 같은 5단계 프로젝트 에디터 흐름을 사용하도록 맞췄다.
- [2026-04-16] 홈의 `이번 달 인건비 확인` 카드가 `PM 입력 금액 필요`, `Projection 없음`, `금액 불일치`, `잔액 부족 경고`를 직접 보여주도록 바뀌어, payroll 이상 신호를 홈에서도 바로 읽을 수 있게 했다.
- [2026-04-16] 포털 홈에 `이번 달 인건비 확인` 상시 CTA 카드를 추가하고, 후보 없음/PM 검토/Admin 최종 확정/지급 확정 완료 상태를 같은 카드에서 읽을 수 있게 정리했다.
