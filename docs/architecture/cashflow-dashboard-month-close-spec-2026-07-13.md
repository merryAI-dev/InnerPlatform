...
| 주차별 합계 | 시트 `C:BN` | 서버가 전체 주차를 합산한다. `BO`는 시트 수식 결과와 대조하는 control total로 보존한다. |
...
| 세금계산서 발행일 | `C7:BN7` | 주차별 원천값 |
| 입금일 | `C8:BN8` | 주차별 원천값 |
| 입금액 | `C9:BN9` | 주차별 원천값 |
| 입금 control total | `BO9` | 현재 원본 수식 `=G9`; 변경하지 않고 전체 주차 합계와 대조 |
| 미지급 control total | `BP9` | 현재 원본 수식 `='예산총괄시트'!F27-BO9`; 미지급 산식 확정 전 표시용 |
| Projection | `C14:BN32` | 주차별 값, `BO14:BO32` control total |
| Actual | `C37:BN55` | 주차별 값, `BO37:BO55` control total |

확인된 원본 특성:

- 월 헤더는 `D:I:N:...:BG`, 주차 데이터는 `C:BN`, 합계는 `BO`, 미지급은 `BP`다.
