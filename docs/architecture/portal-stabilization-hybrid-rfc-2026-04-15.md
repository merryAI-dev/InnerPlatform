...
목표:
- admin summary/dashboard도 동일한 read model 원칙으로 맞춘다.
...
종료 기준:
- summary/dashboard class 화면은 fetch-first and BFF-mediated
- realtime은 board/comment/notification class로 제한

## 9. 성공 지표

- `/portal` 진입 후 Firestore `Listen 400` 반복 재현 0
- 포털 첫 진입 network request 수 유의미하게 감소
- provider mount 수 감소
- 포털 화면 flicker/repaint 제보 감소
- 운영 화면별 데이터 로딩 계약이 API 문서로 명시됨
- 신규 포털 기능이 direct Firestore query 없이 추가 가능

## 10. 실패 지표

- route마다 또 다른 ad-hoc safe fetch 예외를 계속 추가함
- provider를 나누지 않은 채 query helper만 계속 손봄
- BFF read model 없이 Firestore query 튜닝으로만 버팀
- 신규 포털 화면이 다시 raw collection 직접 접근을 시작함

## 11. 리스크와 대응

### 리스크 1. BFF가 점점 비대해질 수 있음

대응:
- endpoint를 화면 단위 read model로 제한
- shared aggregation helper를 분리

### 리스크 2. Firestore raw model과 BFF read model drift

대응:
- raw-to-read-model mapping 테스트 추가
- summary endpoint contract snapshot 유지

### 리스크 3. 이행 중 hybrid 상태 장기화
...
1. 6~8주 실행 플랜
2. portal route-scoped provider 분리 설계
3. read model endpoint 계약 초안
4. realtime allowlist 정책 문서

## 13. 결론

장기적으로 가장 현실적인 방향은 `Firestore 유지 + BFF/API-first hybrid`다.

이 방향은 지금 제품을 다시 안정화하면서도, 나중에 더 큰 서버 권위 구조로 넘어갈 수 있는 중간다리 역할을 한다.

핵심은 Firestore를 버리느냐가 아니라, `클라이언트가 데이터 접근 정책을 얼마나 덜 갖게 만들 것이냐`다.
