...
- Vercel project-level routes redirect public production direct hosts to `https://myscube.myscguard.app`. The internal stage alias `inner-platform-stage-merryai-devs-projects.vercel.app` is intentionally excluded so stage deploys and verification do not traverse the production security domain.
