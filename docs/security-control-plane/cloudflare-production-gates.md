...
- `npm run security:edge-smoke:strict` passed after Cloudflare apply and Vercel route cleanup. Main-push CI also uploads the resulting `tmp/edge-smoke/cloudflare-edge-smoke.json` as the `cloudflare-edge-smoke` artifact; CI allows Cloudflare `403` root-page challenge/block responses from GitHub-hosted runners while preserving all redirect and scanner-path checks.
...
- Vercel project-level routes redirect public production direct hosts to `https://myscube.myscguard.app`. The internal stage alias `inner-platform-internal-stage-merryai-devs-projects.vercel.app` is intentionally excluded so stage deploys and verification do not traverse the production security domain.
