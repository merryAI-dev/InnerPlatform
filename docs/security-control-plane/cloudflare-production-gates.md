...
- `npm run security:edge-smoke:strict` passed after Cloudflare apply and Vercel route cleanup. Main-push CI also uploads the resulting `tmp/edge-smoke/cloudflare-edge-smoke.json` as the `cloudflare-edge-smoke` artifact.
