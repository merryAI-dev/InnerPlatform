...
최근 PM 포털 운영 흐름을 기준으로 다음 개선이 반영되었습니다.

| 영역 | 변경 내용 | 실사용 포인트 |
| --- | --- | --- |
| `사업비 입력(주간)` | 직접 입력형 / 잔액 조정형 흐름 정리, 검토 패널과 값 출처 표시 강화 | 어떤 값이 업로드/수동/계산인지 더 빨리 판단 가능 |
| `사업 배정 수정` | 사업 검색 강화, 선택한 사업 우선 정렬, 최근 사용한 사업 바로가기 추가 | 사업이 많아도 빠르게 찾고 주사업을 바꿀 수 있음 |
| `예산 편집` | `예산 항목 구조 관리`에서 엑셀 붙여넣기 및 CSV 가져오기 지원 | 비목/세목 구조를 한 줄씩 다시 만들지 않아도 됨 |
| 포털 공통 UX | 버튼을 가리던 플로팅 도움말 제거 | `다음`, `저장` 같은 핵심 액션 충돌 감소 |
| 저장 안정성 | 프로젝트 등록/수정 시 Drive 작업과 저장 로직 분리 | Drive 연결 이슈가 있어도 저장 자체가 덜 실패함 |
| 정산 반영 | 매입부가세 반영 및 수동 수정 보존 강화 | 사용자가 지운 값이나 0 입력이 저장 후 다시 덮어써질 가능성 감소 |

### 이번 버전에서 먼저 확인하면 좋은 것

1. `사업 배정 수정`에서 검색으로 사업을 바로 찾을 수 있는지
2. 최근 사용한 사업이 상단 추천으로 다시 보이는지
3. `사업비 입력(주간)`에서 값 출처와 검토 필요 상태가 읽히는지
4. `예산 편집 > 구조 관리`에서 엑셀 두 열 복붙 또는 CSV 가져오기가 자연스러운지
