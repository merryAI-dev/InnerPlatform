...
이 문서는 비개발자 PM이 MYSC 포털을 처음 사용할 때, 실제 업무 흐름대로 차근차근 따라올 수 있도록 만든 상세 온보딩 자료입니다.
...
이 앱은 “AI가 다 해주는 앱”이 아니라, 반복 작업에서 PM의 판단을 빼앗긴 지점만 자동화해 주는 운영 도구예요.
...
그리고 가장 먼저 기억할 구분이 있어요.

| 구분 | 의미 |
| --- | --- |
| 시스템이 잘하는 일 | 파일 읽기, 표 정렬, 매핑, 계산, 반영 연결, 상태 표시 |
| 사람이 꼭 판단해야 하는 일 | 주차 판단, 항목 분류, 설명 정합성, 마감 타이밍 결정 |
...
로그인 후 바로 입력으로 가기보다, 첫 3분은 사업 배정 점검이 기본입니다.
...
퇴근 전 아래만 확인하면 한 주의 리스크가 많이 줄어요.

| 항목 | 완료 여부 |
| --- | --- |
| 통장내역 업로드가 누락 없이 저장되었는지 |  |
| 사업비 입력에서 핵심 칼럼이 모두 채워졌는지 |  |
| 저장 후 캐시플로 Actual 반영이 확인되었는지 |  |
| Projection 월 계획이 저장되었는지 |  |
| 작성완료 대상 주차를 넘겼는지 |  |
| 내 제출 현황에서 미작성 주차가 없는지 |  |
| 확인요청 대응이 필요한 건을 정리했는지 |  |
