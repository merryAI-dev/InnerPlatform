...
- 분할 패널에서만 `CashflowProjectSheet`의 additive compact layout을 사용하고 기존 route의 데이터·업무 로직과 기본 layout은 유지한다.

## 성공 기준

- [x] 각 사업 행의 두 주차 카드가 같은 행에 나란히 표시된다.
- [x] `사업 보기` 클릭 후 pathname은 `/cashflow/export`로 유지되고 선택 사업 query만 추가된다.
- [x] 1024px 이상에서 내보내기 화면과 사업 상세가 각각 절반을 차지한다.
- [x] 375px에서 사업 상세가 전체 화면으로 열리고 닫기 버튼과 Escape로 닫힌다.
- [x] 브라우저 뒤로가기가 패널을 닫고, 패널을 열기 전 필터·선택·스크롤 상태를 유지한다.
- [x] 패널이 닫혀 있을 때 사업 상세 provider와 상세 데이터 요청이 발생하지 않는다.
- [x] 사업 명부가 비어 있지 않아 유효성 판정이 가능한 경우, 존재하지 않거나 현재 명부에 없는 project query는 패널을 열지 않고 URL에서 제거된다. 명부 0건과 초기 로딩을 구분할 별도 전역 상태는 추가하지 않고 두 경우 모두 패널·상세 요청을 차단한다.
- [x] 기존 다운로드 요청 body·파일명·XLSX 결과와 운영 현황 BFF 요청 수가 변하지 않는다.
...
- 사업 상세 화면 내부 데이터·권한·mutation 계약 변경
